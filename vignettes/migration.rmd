...
`.on_load()`, which is a function that gets executed whenever a module
is first loaded inside an R session.
...
[`box::name()`](https://klmr.me/box/reference/name.html). It no longer
has any arguments.
...
to declare them explicitly (e.g. via `box::use(stats)`).
