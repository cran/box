...
### “object ‘lm’ not found” inside a module
...
To use names from any of these packages, the packages needs to be loaded
via `box::use` same as third-party packages. Alternatively, the standard
module `r/core` can be used (and, optionally, attached):
...
This module ships with ‘box’ and is always findable, regardless of the
value of the module search path.

### “there is no package called ‘foo’” when running `box::use(foo)`, even though the module ‘foo’ exists
...
path (set via the environment variable `R_BOX_PATH` or
