...
---
> **NOTE:** The contents of this vignette refer to a *development
> version* of ‘box’! They may not apply entirely to the last stable
> release.
...
if the function is invoked from code that wasn’t loaded via `box::use()`
(as is the case here), its value is `NULL`. In other words,
...
[2] Depending on how it’s invoked; but we shouldn’t make assumptions. In
particular, when invoked as a job in RStudio, the test module is loaded
via the test helper, and thus the module internals are not visible.
